...
## 10. Adding Capability-Based UI Controls (Frontend)

The frontend uses camera capabilities from the backend to conditionally render controls. This ensures users only see controls their camera hardware supports.

### Step 1: Check if capability exists in types

**Location**: `frontend/src/api/types.ts`

Review the `CameraCapabilities` interface. If your capability isn't listed, add it:

```typescript
export interface CameraCapabilities {
  // ... existing capabilities ...
  MyNewControl?: boolean;  // Add your capability
}
```

### Step 2: Use capability in settings component

**Location**: Component in `frontend/src/components/settings/`
...
interface MySettingsProps {
  ...
  capabilities?: CameraCapabilities;  // Add capabilities prop
}

export function MySettings({ config, onChange, getError, capabilities }: MySettingsProps) {
  // Conditionally render based on capability
  if (!capabilities?.MyNewControl) {
    return (
      <div className="text-gray-400 text-sm">
        This feature is not supported by your camera.
      </div>
    );
  }

  return (
    <FormSection title="My Feature">
      <FormSlider
        label="My Setting"
        value={config.my_setting?.value as number ?? 50}
        onChange={(v) => onChange('my_setting', v)}
        // ...
      />
    </FormSection>
  );
...
### Step 3: Pass capabilities from Settings page

**Location**: `frontend/src/pages/Settings.tsx`

The `useCameraCapabilities` hook is already integrated. Pass capabilities to your component:
...
### Step 4: Test with different cameras

- **Pi Camera v2 (IMX219)**: Fixed focus, no autofocus
- **Pi Camera v3 (IMX708)**: Full autofocus support
- **Pi Camera HQ (IMX477)**: Manual focus only

### Common Capability Checks

| Capability | Use Case |
|------------|----------|
| `AfMode` | Show autofocus mode selector |
| `LensPosition` | Show manual focus slider |
| `AwbMode` | Show white balance mode selector |
| `Brightness`, `Contrast`, etc. | Show image adjustment controls |
| `HdrMode` | Show HDR toggle (Pi Camera v3) |

### Pattern: Multiple Capability Checks

```tsx
// Show section only if ANY of these are supported
const hasImageAdjustments = capabilities?.Brightness ||
...
// Render individual controls based on specific capabilities
return (
  <FormSection title="Image Adjustments">
    {capabilities?.Brightness && (
      <FormSlider label="Brightness" ... />
    )}
    {capabilities?.Contrast && (
      <FormSlider label="Contrast" ... />
    )}
    {capabilities?.Saturation && (
      <FormSlider label="Saturation" ... />
    )}
  </FormSection>
);
```
