...
| `/0/status.json` | webu_json | System status + camera capabilities |
...
The frontend dynamically detects camera hardware capabilities to conditionally render controls that are only relevant for specific camera models.
...
interface CameraCapabilities {
  // Autofocus controls
  AfMode?: boolean;        // Autofocus mode (auto/manual/continuous)
  LensPosition?: boolean;  // Manual focus position
  AfRange?: boolean;       // Focus range (normal/macro/full)
  AfSpeed?: boolean;       // Focus speed setting

  // White balance
  AwbEnable?: boolean;     // AWB on/off
  AwbMode?: boolean;       // AWB mode selection
  ColourGains?: boolean;   // Manual color gains
...
function LibcameraSettings({ config, capabilities }) {
  // Show autofocus section only if camera supports it
  if (capabilities?.AfMode) {
    return (
      <FormSection title="Autofocus">
        <FormSelect label="Mode" options={AF_MODES} ... />
        {capabilities?.LensPosition && (
          <FormSlider label="Lens Position" ... />
        )}
      </FormSection>
    );
  }

  // Fallback for cameras without autofocus
  return (
    <div className="text-gray-400">
      Autofocus: Not supported. This camera has fixed focus.
    </div>
  );
}
```
...
| Camera | AfMode | LensPosition | Notes |
|--------|--------|--------------|-------|
| Pi Camera v2 (IMX219) | ❌ | ❌ | Fixed focus |
| Pi Camera v3 (IMX708) | ✅ | ✅ | Full AF support |
