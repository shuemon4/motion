...
The frontend uses camera capabilities from `status.json` to conditionally show/hide controls based on hardware support.

**Hook Pattern**:
```typescript
// hooks/useCameraCapabilities.ts
export function useCameraCapabilities(cameraId: number) {
  return useQuery({
    ...
    staleTime: 60000, // Capabilities don't change at runtime
  });
}
```
...
**Component Usage**:
```tsx
function LibcameraSettings({ config, onChange, capabilities }: Props) {
  return (
    <>
      {/* Only show if camera supports autofocus */}
      {capabilities?.AfMode ? (
        <FormSelect label="Autofocus Mode" ... />
      ) : capabilities !== undefined ? (
        <div className="text-gray-400">
          Autofocus not supported on this camera.
        </div>
      ) : null}
    </>
  );
...
**Key Principles**:
- Return `null` while capabilities are loading (avoid layout shift)
- Show informative message when feature not supported
- Always provide fallback for graceful degradation

### Recording Mode Translation

Frontend uses semantic mode names that map to multiple backend parameters:
...
// Component applies all related parameters when mode changes
const handleModeChange = (mode: string) => {
  const params = RECORDING_MODE_MAP[mode];
  Object.entries(params).forEach(([key, value]) => onChange(key, value));
};
```

### Parameter Mapping Pattern

Frontend uses mapping objects to translate between Motion parameters and UI options:

```typescript
// utils/parameterMappings.ts
export const ENCODER_PRESETS = [
  { value: 'ultrafast', label: 'Ultrafast', description: 'Lowest CPU' },
  { value: 'medium', label: 'Medium (Default)', description: 'Balanced' },
  { value: 'veryslow', label: 'Very Slow', description: 'Highest quality' },
];
...
// Helper functions for container type checking
