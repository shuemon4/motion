...
The React frontend exposes movie format options through the Settings → Movie Settings panel.
...
|----------|-------|-------|
| MKV (Matroska) | `mkv` | Default, recommended |
| MP4 | `mp4` | Most compatible |
| MOV (QuickTime) | `mov` | Apple ecosystem |
| 3GP (Mobile) | `3gp` | Legacy mobile |
| MKV - H.264 Hardware (Pi 4) | `mkv:h264_v4l2m2m` | Low CPU |
| MP4 - H.264 Hardware (Pi 4) | `mp4:h264_v4l2m2m` | Low CPU |
| MKV - H.264 Software | `mkv:libx264` | Explicit software |
| MP4 - H.264 Software | `mp4:libx264` | Explicit software |
| MKV - H.265 Software (high CPU) | `mkv:libx265` | Smaller files |
| HEVC/H.265 in MP4 (high CPU) | `hevc` | Smaller files |
| WebM (VP8) | `webm` | Web streaming |

**Encoder Preset Dropdown** (hidden for hardware/passthrough/webm):
| UI Label | Value | CPU Usage |
|----------|-------|-----------|
| Ultrafast | `ultrafast` | ~35-40% |
| Superfast | `superfast` | ~40-45% |
| Very Fast | `veryfast` | ~45-50% |
| Faster | `faster` | ~50-55% |
| Fast | `fast` | ~50-55% |
| Medium (Default) | `medium` | ~50-60% |
| Slow | `slow` | ~60-70% |
| Slower | `slower` | ~70-80% |
| Very Slow | `veryslow` | ~70-80% |

### Recording Modes

The UI provides three recording modes that map to Motion parameters:

| Mode | movie_output | movie_output_motion | emulate_motion |
|------|--------------|--------------------:|----------------|
| Motion Triggered | `true` | `true` | `false` |
| Continuous | `true` | `false` | `true` |
| Off | `false` | - | `false` |

**Continuous Recording** (`emulate_motion = on`):
- Records 24/7 regardless of motion detection
- Motion detection still runs (for overlays)
- Significant CPU usage for encoding
- Recommended settings for Pi 5:
  - Encoder preset: `ultrafast` or `superfast`
  - Consider active cooling
  - Or use passthrough if source is H.264

### Device-Aware Warnings

The UI shows contextual warnings based on codec selection:

- **Hardware codec selected**: Info about Pi 4 requirement, Pi 5 fallback
- **HEVC/H.265 selected**: High CPU warning (80-100%)
- **Continuous recording**: CPU usage expectations by device type
- **WebM selected**: Info that encoder preset doesn't apply to VP8
